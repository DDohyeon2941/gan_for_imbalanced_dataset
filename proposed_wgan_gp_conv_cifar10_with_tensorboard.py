...
minority_data = [(data, label) for data, label in train_dataset if label in [5,]][:500]
...
writer = SummaryWriter('gan_runs/GAN_experiment')
...
for epoch in range(num_epochs):
    for i, (real_data, _) in enumerate(minority_loader):
        ...
        for _ in range(5):
            ...
            d_loss = -torch.mean(d_loss_real) + (0.9*torch.mean(d_loss_fake_min)) + gradient_penalty_min + (0.1*torch.mean(d_loss_fake_maj)) + (0.2 * output_diff.mean())
        ...
        g_loss = -(0.9*torch.mean(output_min)) - (0.1*torch.mean(output_maj))
   ...
   # Epoch마다 생성된 이미지들을 TensorBoard에 기록합니다.
    with torch.no_grad():
        # 고정된 잠재 벡터를 사용하여 일관성 있는 이미지를 생성합니다.
        # 예를 들어, 스칼라 값 로깅
        writer.add_scalar('generator loss', g_loss.item(), epoch * len(minority_loader) + i)
        writer.add_scalar('discriminator loss', d_loss.item(), epoch * len(minority_loader) + i)

        fixed_z = torch.randn(64, z_dim, 1, 1).to(device)
        fake_images = generator(fixed_z).detach().cpu()
        img_grid = torchvision.utils.make_grid(fake_images, normalize=True, scale_each=True)
