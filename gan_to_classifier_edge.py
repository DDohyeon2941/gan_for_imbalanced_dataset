...
# 학습률 조정을 위한 함수
def adjust_learning_rate(optimizer, epoch, initial_lr, adjust_epoch, decay_rate):
    if epoch % adjust_epoch == 0:
        new_lr = initial_lr * (decay_rate ** (epoch // adjust_epoch))
...
def train_wgan(data_dir = './data',
    ...
    for epoch in range(num_epochs):
        ...
        adjust_learning_rate(g_optimizer, epoch, lr, 50, 0.5)  # 생성자 학습률 조정
        adjust_learning_rate(d_optimizer1, epoch, lr, 50, 0.5)  # 판별자 학습률 조정
