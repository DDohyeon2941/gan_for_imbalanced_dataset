...
class MultiLayerProjection(nn.Module):
    ...
    def forward(self, x, labels):
        # 각 클래스에 대한 트랜스포메이션 적용
        outputs = torch.zeros(x.size(0), self.output_dim).to(x.device)
        for c in range(self.num_classes):
            class_mask = (labels == c)
            if class_mask.any():
                outputs[class_mask] = self.class_transforms[c](x[class_mask])
        return outputs


# 클래스 간 구분 손실 함수
def inter_class_separation_loss(projected_vectors, labels, num_classes):
    class_means = []
    for c in range(num_classes):
        class_mask = (labels == c)
        class_vectors = projected_vectors[class_mask]
    ...
    for i in range(num_classes):
        for j in range(i + 1, num_classes):
            loss += torch.norm(class_means[i] - class_means[j])

    return loss / (num_classes * (num_classes - 1) / 2)

####
class GeneratorWithProjection(nn.Module):
    def __init__(self, decoder, latent_vector_generator, projection_model):
        super(GeneratorWithProjection, self).__init__()
        self.latent_vector_generator = latent_vector_generator
    ...
    def forward(self, labels):
        ...
        # 다층 프로젝션 적용
        projected_z = self.projection_model(z)

        # Decoder를 통한 이미지 생성
        projected_z = projected_z.view(projected_z.size(0), -1, 4, 4)
        return self.decoder(projected_z)
...
#%%
latent_dim = 256*4*4  # 잠재 벡터 차원
hidden_dims = [512, 256]  # 중간 층의 차원
num_classes = 10  # 클래스 수
# 예시: 모델 초기화 및 학습 설정
projection_model = MultiLayerProjection(input_dim=latent_dim, hidden_dims=hidden_dims, output_dim=latent_dim, num_classes=num_classes)
# ... (필요한 파라미터 설정)

optimizer = optim.Adam(projection_model.parameters(), lr=0.001)
...
for epoch in range(num_epochs):
    for _, labels in data_loader:
        ...
        # 잠재 벡터 생성
        with torch.no_grad():
            latent_vectors = latent_vector_generator.sample(labels)

        # 다층 프로젝션 적용
        projected_vectors = projection_model(latent_vectors)

        # 재구성 손실
        reconstruction_loss = reconstruction_loss_func(projected_vectors, latent_vectors)

        # 클래스 간 구분 손실
        separation_loss = inter_class_separation_loss(projected_vectors, labels, num_classes)

        # 전체 손실
        lambda_factor = 0.1
        total_loss = reconstruction_loss + lambda_factor * separation_loss
...
generator = nn.DataParallel(GeneratorWithProjection(autoencoder.module[1], latent_vector_generator, projection_model)).to(device)
